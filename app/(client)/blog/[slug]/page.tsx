        ...
        <div className="md:col-span-3">
          ...
          <div>
            <div className="text-xs flex items-center gap-5 my-7">
              ...
              <p className="flex items-center gap-1 text-lightColor relative group hover:cursor-pointer hover:text-shop_dark_green hoverEffect">
                <Calendar size={15} />{" "}
                {dayjs(blog.publishedAt ?? blog._createdAt).format("MMMM D, YYYY")}
                <span className="absolute left-0 -bottom-1.5 bg-lightColor/30 inline-block w-full h-[2px] group-hover:bg-shop_dark_green hoverEffect" />
              </p>
            </div>
            <h2 className="text-2xl font-bold my-5">{blog?.title ?? "Untitled"}</h2>
      ...
      <div className="border border-lightColor p-5 rounded-md">
        ...
        <div className="space-y-2 mt-2">
          {(categories ?? []).map((cat, index) => {
            const title = cat?.blogcategories?.[0]?.title ?? "Uncategorize";
            const key = cat?.blogcategories?.[0]._id ?? index;
            return(
              <div
              key={key}
              className="text-lightColor text-sm font-medium flex justify-between items-center"
              >
                <p>{title}</p>
      ...
      <div className="border border-lightColor p-5 rounded-md mt-10">
        ...
        <div className="space-y-4 mt-4">
           {(blogs ?? []).map((b, index) => {
            ...
            const slugCurrent = b?.slug?.current;
            
            if (!href) return null;
            const key = slugCurrent ?? index;

            return (
              <Link href={href} key={key} className="flex items-center gap-2 group">
                {b?.mainImage?.asset && (
                  <Image
                    src={urlFor(b.mainImage).url()}
                    alt={b.title ?? "blogImage"}
                    width={100}
                    height={100}
